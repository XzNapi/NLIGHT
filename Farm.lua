return function(Core)
                    ...
                    if #targetList > 0 then
                        if farmPhase == "PLACE" then
                            ...
                            for i = 1, #targetList do
                                ...
                                if not hasBlock then
                                    ...
                                    if targetStringID == "auto" or targetStringID == "" then
                                        local held = Core.Utils.getHeldItem(); if held then targetStringID = held end
                                    end
                            ...
                            if itemHabis then
                                ...
                                isOutOfItems = true; farmPhase = "BREAK"
    ...
    -- =========================================================================
    -- AUTO PLANTER (SWEEP LINE SMOOTH LOGIC)
    -- =========================================================================
    task.spawn(function()
        while task.wait() do
            pcall(function()
                if Core.Toggles.autoPlanter and Core.Managers.MovementState and Core.Remotes.PlayerPlaceRemote then
                    ...
                    if #validSpots > 0 then
                        ...
                        -- Mengurutkan posisi menanam berdasarkan sisi terdekat karakter (Mulus Kiri-Kanan / Kanan-Kiri)
                        local avgX = 0
                        for _, s in ipairs(rowSpots) do avgX = avgX + s.x end
                        avgX = avgX / #rowSpots
                        local pX = pPos.X / Core.Utils.TILE_SIZE
                        if pX > avgX then table.sort(rowSpots, function(a, b) return a.x > b.x end) else table.sort(rowSpots, function(a, b) return a.x < b.x end) end
                        
                        local moveSpeed = tonumber(Core.Inputs["planterSpeedBox"] and Core.Inputs["planterSpeedBox"].Text or "45") or 45
                        
                        -- SAPU BERSIH 1 BARIS LURUS TANPA JEDA SCAN
                        for _, targetSpot in ipairs(rowSpots) do
                            if not Core.Toggles.autoPlanter then break end
                            
                            -- Pengecekan item realtime sebelum meletakkan blok
                            ...
                            if Core.Managers.InventoryModule and Core.Managers.InventoryModule.Stacks then
                                ...
                                for j = 1, (Core.Managers.InventoryModule.MaxSlots or 100) do
                                    local stackInfo = Core.Managers.InventoryModule.Stacks[j]
                                    if stackInfo and stackInfo.Id and stackInfo.Amount and stackInfo.Amount > 0 then
                                        local sID = string.lower(tostring(stackInfo.Id))
                                        local sName = sID
                                        if Core.Managers.ItemsManager and Core.Managers.ItemsManager.ItemsData and Core.Managers.ItemsManager.ItemsData[tostring(stackInfo.Id)] then
                                            sName = string.lower(tostring(Core.Managers.ItemsManager.ItemsData[tostring(stackInfo.Id)].Name or sID))
                                        end
                                        if sID == plantStr or sName == plantStr then exactMatch = j break
                                        elseif (string.find(sID, plantStr) or string.find(sName, plantStr)) and not partialMatch then partialMatch = j end
                                    end
                                end
                                currentSlot = exactMatch or partialMatch
                            end
                            
                            if not currentSlot then
                                print("[NLight] Auto Planter: Item habis!"); Core.Toggles.autoPlanter = false; if updatePlanterToggle then updatePlanterToggle() end; break
                            end

                            if Core.Pathfinding.aiMoveTo(targetSpot.x, targetSpot.y, moveSpeed, "autoPlanter") then
                                Core.Remotes.PlayerPlaceRemote:FireServer(Vector2.new(targetSpot.x, targetSpot.y), currentSlot)
                                task.wait(0.02) -- Waktu tunggu yang sangat pendek agar mengalir mulus
                            else
    ...
    -- =========================================================================
    -- AUTO BREAKER (SWEEP LINE SMOOTH LOGIC)
    -- =========================================================================
    task.spawn(function()
        while task.wait() do
            pcall(function()
                if Core.Toggles.autoBreaker and Core.Managers.MovementState and Core.Remotes.PlayerFistRemote then
                    ...
                    if #validSpots > 0 then
                        ...
                        -- SAPU BERSIH 1 BARIS LURUS TANPA JEDA SCAN
                        for _, targetSpot in ipairs(rowSpots) do
                            if not Core.Toggles.autoBreaker then break end
                            ...
                            if Core.Pathfinding.aiMoveTo(targetSpot.x, targetSpot.y, moveSpeed, "autoBreaker") then
                                ...
                                task.wait(0.02) -- Waktu tunggu sangat pendek agar mengalir
                            else
                                Core.Pathfinding.blacklistedSpots[targetSpot.x..","..targetSpot.y] = true
                            end
                        end
